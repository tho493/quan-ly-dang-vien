...
function checkPermission($user, $str_controller){
	if (isset($user) && (strpos($user['Permission'], $str_controller) !== false)) {		
		return true;
	} else {return false;}
}
function array2d_sum(array $arr) {
    $sum = array_sum($arr);
    foreach($arr as $child) {
        $sum += is_array($child) ? array2d_sum($child) : 0;
    }
    return $sum;
}